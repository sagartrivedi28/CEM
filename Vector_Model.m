...
% gdslist = {tlist.name};
% 
% sr_path3 = 'D:\SAGAR\WORK SPACE\SAGAR\AIMS\Main Project\AIMS simulation project\Production cases(N16)\SEM Images\SEM_ST';           % SEM Image
% sr_name3 = '10-ST.tif'; 
% tlist = dir(fullfile(sr_path3,'*.tif'));
% sempath = sr_path3;
% semlist = {tlist.name};
% 
% sr_path = 'D:\SAGAR\WORK SPACE\SAGAR\AIMS\Main Project\AIMS simulation project\Production cases(N16)\AIMS_mode\AIMS_ST'; % AIMS Mode image
% sr_name = '10_ST.csv';
% tlist = dir(fullfile(sr_path,'*.csv'));
% Aamspath = sr_path;
% Aamslist = {tlist.name};
% 
% sr_pathS = 'D:\SAGAR\WORK SPACE\SAGAR\AIMS\Main Project\AIMS simulation project\Production cases(N16)\Scanner_mode\Scanner_ST'; % Scanner Mode image
% sr_nameS = '10_ST.csv';
% tlist = dir(fullfile(sr_path,'*.csv'));
% Samspath = sr_pathS;
% Samslist = {tlist.name};
...
% % [TCC4_ams, lm_ams, TCC2_ams,slm_ams] = Vector_TCC(Paims,Jsource,Mypara,false);
% ATCC_basisxx = {TCC2_ams{1,:}}; ATCC_basisxy = {TCC2_ams{2,:}}; Alm = slm_ams;
% % save('MxTCC_ams.mat','TCC4_ams','lm_ams','TCC2_ams','slm_ams');
% 
% % [TCC4_scanner, lm_scanner, TCC2_scanner,slm_scanner] = Vector_TCC(Pscanner,Jsource,Mypara,false);
% STCC_basisxx = {TCC4_scanner{1,:}}; STCC_basisxy = {TCC4_scanner{2,:}}; 
% STCC_basisyx = {TCC4_scanner{3,:}}; STCC_basisyy = {TCC4_scanner{4,:}}; 
% Slm = lm_scanner;
% % save('MxTCC_scaneer.mat','TCC4_scanner','lm_scanner','TCC2_scanner','slm_scanner');
% 
...
for tf=1:100
    ATCC_basisfinal{tf}=ATCC_basisxx{tf}(21:end-20,21:end-20) + ATCC_basisxy{tf}(21:end-20,21:end-20);    
    STCC_basisfinal{tf}=STCC_basisxx{tf}(21:end-20,21:end-20) + STCC_basisxy{tf}(21:end-20,21:end-20)+...
                        STCC_basisyx{tf}(21:end-20,21:end-20) + STCC_basisyy{tf}(21:end-20,21:end-20);
end;

Sord = 42;
ATCCweight=0;
for k=1:Sord
    ATCCweight = ATCCweight + Alm(k)*sum(sum(abs(ATCC_basisfinal{k}))).^2;
end;
Alm = Alm(1:Sord)/ATCCweight;

STCCweight=0;
for k=1:(Sord)
    STCCweight = STCCweight + Slm(k)*sum(sum(abs(STCC_basisfinal{k}))).^2;
end;
Slm = 1.3518*(Slm(1:(2*Sord))/STCCweight); % Scanner Gain
...
for k=1:Sord
    my_aimst = my_aimst + Alm(k)*(conv2(mask_ts2,(ATCC_basisfinal{k}),'same')).^2;
end;
...
while((max(max(abs(AMS_sim(21:end-20,21:end-20)-AMS_org(21:end-20,21:end-20)))))>0.015)
    Mweight=zeros(size(mask_ts3));
    Icost = Region1.*(AMS_sim-AMS_org);%.*(AMS_sim.*(1-AMS_sim));
%     diff_gif(:,:,int+1)=Icost;
    for k=1:Sord
        Tmfr = Icost.*(conv2(conj(mask_ts3),conj(ATCC_basisfinal{k}),'same'));
        Mweight = Mweight + Alm(k)*(conv2(Tmfr,rot90((ATCC_basisfinal{k}),2),'same'));
    end;
    
    fact = (max(abs(Icost(:))))/(max(abs(Mweight(:))));
    old_mask = mask_ts3;
    ...
    for k=1:Sord
        AMS_sim = AMS_sim + (Alm(k))*(conv2(mask_ts3,ATCC_basisfinal{k},'same')).^2;
    end;
...
for k=1:(2*Sord)
    Scanner_sim = Scanner_sim + (Slm(k))*(conv2(mask_ts3,STCC_basisfinal{k},'same')).^2;
end;

Scanner_sim = (abs(Scanner_sim(21:end-20,21:end-20))); 
Nsize = size(Scanner_sim);
...
MasterImg = [AMS_sim(21:end-20,21:end-20) AMS_org(21:end-20,21:end-20) Scanner_sim Scanner_org(21:end-20,21:end-20)];
imwrite(uint8(round(255*abs(MasterImg))),colormap(jet(255)),fullfile(gdspath,'VectorImage',['MasterI_' gdslist{No} '_.bmp']));

MasterDiff = [abs(AMS_sim(21:end-20,21:end-20)-AMS_org(21:end-20,21:end-20)) abs(Scanner_sim-Scanner_org(21:end-20,21:end-20)) ...
    abs(AMS_sim(21:end-20,21:end-20)-Scanner_sim) abs(AMS_org(21:end-20,21:end-20)-Scanner_org(21:end-20,21:end-20))];
SV_OV = max(max(abs(Scanner_sim-Scanner_org(21:end-20,21:end-20)))); SS_SV = max(max(abs(AMS_sim(21:end-20,21:end-20)-Scanner_sim)));
OS_OV = max(max(abs(AMS_org(21:end-20,21:end-20)-Scanner_org(21:end-20,21:end-20))));
...
tempA(Aind(1)+1:Aind(1)+Nsize(1),Aind(2)+1:Aind(2)+Nsize(2)) = AMS_sim(21:end-20,21:end-20);
...
tempA(Aind(1)+1:Aind(1)+Nsize(1),Aind(2)+1:Aind(2)+Nsize(2)) = Scanner_sim;
