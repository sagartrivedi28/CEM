function [TCC4_ams, lm, TCC2_ams,slm] = Vector_TCC(Paims,Jsource,Mypara,Flagfast)
...
for k=1:base_ord
    TCC_freqxx{k} = imresize(reshape(U1(1:original_sz(1)^2,k),original_sz),Sampleup);
    TCC_basisxx{k}  = ifftshift(ifft2(fftshift(padarray(TCC_freqxx{k},round((new_sz-Sampleup*original_sz)/2)))));
    
    TCC_freqxy{k} = imresize(reshape(U1(1+original_sz(1)^2:2*original_sz(1)^2,k),original_sz),Sampleup);
    TCC_basisxy{k}  = ifftshift(ifft2(fftshift(padarray(TCC_freqxy{k},round((new_sz-Sampleup*original_sz)/2)))));
    
    TCC_freqyx{k} = imresize(reshape(U1(1+2*original_sz(1)^2:3*original_sz(1)^2,k),original_sz),Sampleup);
    TCC_basisyx{k}  = ifftshift(ifft2(fftshift(padarray(TCC_freqyx{k},round((new_sz-Sampleup*original_sz)/2)))));
    
    TCC_freqyy{k} = imresize(reshape(U1(1+3*original_sz(1)^2:4*original_sz(1)^2,k),original_sz),Sampleup);
    TCC_basisyy{k}  = ifftshift(ifft2(fftshift(padarray(TCC_freqyy{k},round((new_sz-Sampleup*original_sz)/2)))));
end;
TCC4_ams = {TCC_basisxx{:}; TCC_basisxy{:}; TCC_basisyx{:}; TCC_basisyy{:}};
...
for k=1:base_ord
    STCC_freqxx{k} = imresize(reshape(SU1(1:original_sz(1)^2,k),original_sz),Sampleup);
    STCC_basisxx{k}  = ifftshift(ifft2(fftshift(padarray(STCC_freqxx{k},round((new_sz-Sampleup*original_sz)/2)))));
    
    STCC_freqxy{k} = imresize(reshape(SU1(1+original_sz(1)^2:2*original_sz(1)^2,k),original_sz),Sampleup);
    STCC_basisxy{k}  = ifftshift(ifft2(fftshift(padarray(STCC_freqxy{k},round((new_sz-Sampleup*original_sz)/2)))));
end;
TCC2_ams = {STCC_basisxx{:}; STCC_basisxy{:}};
